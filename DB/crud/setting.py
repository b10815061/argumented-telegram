...
def create_setting(user_id: str, font_size: int, language: str) -> Setting:
    ...
    with Session(engine) as session:
        setting = Setting(user_id=str(
            user_id), font_size=font_size, language=str(language))
        session.add(setting)
        session.commit()
...
def update_setting(user_id: str, font_size: int, language: str) -> Setting:
    # if not exist then create
    original_setting = get_setting(user_id)
    if (original_setting == None):
        return create_setting(user_id, font_size, language)
    ...
    with Session(engine) as session:
        setting = session.query(Setting) \
            .filter(Setting.user_id == str(user_id)) \
            .update({'font_size': int(font_size), 'language': language})
        session.commit()
        setting = session.query(Setting) \
            .filter(Setting.user_id == str(user_id)) \
            .first()
